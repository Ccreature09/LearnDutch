...
/**
 * Generate a hint that clarifies the subject and verb form needed
 */
function getSubjectHint(subject: Subject, verbForm: string): string {
  if (subject.en.includes("you")) {
    if (subject.en === "you (informal)") {
      return `Use "jij" form: ${subject.nl} ${verbForm}`;
    }
    if (subject.en === "you (formal)") {
      return `Use formal "u" form: ${subject.nl} ${verbForm}`;
    }
    if (subject.en === "you all") {
      return `Use "jullie" (plural you): ${subject.nl} ${verbForm}`;
    }
  }
  return "";
}
...
function collectReferenceTokens(): { dutch: Set<string>; english: Set<string> } {
  const dutch = new Set<string>();
  const english = new Set<string>();

  const addTokens = (value: string | undefined, target: Set<string>) => {
    if (!value) return;
    for (const token of value.toLowerCase().split(/[\s/]+/)) {
      for (const part of token.split(/[-']/)) {
        const cleaned = part.trim().replace(/^[^\p{L}]+|[^\p{L}]+$/gu, "");
        if (cleaned) target.add(cleaned);
      }
      const cleanedWhole = token.trim().replace(/^[^\p{L}]+|[^\p{L}]+$/gu, "");
      if (cleanedWhole) target.add(cleanedWhole);
    }
  };

  const addEntry = (dutchValue: string | undefined, englishValue: string | undefined) => {
    addTokens(dutchValue, dutch);
    addTokens(englishValue, english);
  };

  for (const entry of nounReference) {
    addEntry(entry.dutch, entry.english);
    addEntry(entry.article, entry.thisForm);
    addEntry(entry.article, entry.thatForm);
    addTokens(entry.plural, dutch);
  }

  for (const entry of verbReference) {
    addEntry(entry.infinitive, entry.english);
    addTokens(entry.imperative, dutch);
    addTokens(entry.pastParticiple, dutch);
    addTokens(entry.perfect, dutch);
    addTokens(entry.past.singular, dutch);
    addTokens(entry.past.plural, dutch);
    addTokens(entry.present.ik, dutch);
    addTokens(entry.present.jij, dutch);
    addTokens(entry.present.hij, dutch);
    addTokens(entry.present.wij, dutch);
    addTokens(entry.present.jullie, dutch);
    addTokens(entry.present.zij, dutch);
    if (entry.present.je) addTokens(entry.present.je, dutch);
    if (entry.present.u) addTokens(entry.present.u, dutch);
    if (entry.present.we) addTokens(entry.present.we, dutch);
    if (entry.present.het) addTokens(entry.present.het, dutch);
    if (entry.present.ze) addTokens(entry.present.ze, dutch);
  }

  for (const value of [
    ...subjects.map((item) => item.en),
    ...adjectives.map((item) => item.en),
    ...places.map((item) => item.en),
    ...adverbs.map((item) => item.en),
    ...objects.map((item) => item.en),
    ...modalVerbs.map((item) => item.en),
    ...baseVerbs.map((item) => item.en),
    ...participles.map((item) => item.en),
    ...numbers.map((item) => item.en),
    ...transport.map((item) => item.en),
    ...becauseReasons.map((item) => item.english),
    ...datMainVerbs.map((item) => item.english),
    ...datClauseContents.map((item) => item.english),
    ...frontedComplements.map((item) => item.english),
    ...whileScenarios.map((item) => item.english),
    ...toenEvents.map((item) => item.english),
    ...perfectTimePhrases.map((item) => item.english),
    ...questionTimes,
    ...questionTimesDutch,
    ...modalActions.map((item) => item.english),
    ...travelDestinations.map((item) => item.english),
    ...travelArrivalSpots.map((item) => item.english),
    ...transportOptions.map((item) => item.english)
  ]) {
    addTokens(value, english);
  }

  for (const value of [
    ...subjects.map((item) => item.nl),
    ...adjectives.map((item) => item.nl),
    ...places.map((item) => item.nl),
    ...adverbs.map((item) => item.nl),
    ...objects.map((item) => item.nl),
    ...modalVerbs.map((item) => item.nlInf),
    ...baseVerbs.map((item) => item.nlInf),
    ...participles.map((item) => item.nl),
    ...numbers.map((item) => item.nl),
    ...transport.map((item) => item.nl),
    ...becauseReasons.map((item) => item.dutchWant),
    ...becauseReasons.map((item) => item.dutchOmdat),
    ...datMainVerbs.map((item) => item.dutch),
    ...datClauseContents.map((item) => item.dutch),
    ...frontedComplements.map((item) => item.dutch),
    ...whileScenarios.map((item) => item.dutch),
    ...toenEvents.map((item) => item.dutch),
    ...perfectTimePhrases.map((item) => item.dutch),
    ...modalActions.map((item) => item.dutch),
    ...travelDestinations.map((item) => item.dutch),
    ...travelArrivalSpots.map((item) => item.dutch),
    ...transportOptions.map((item) => item.dutch),
    ...questionTimesDutch
  ]) {
    addTokens(value, dutch);
  }

  return { dutch, english };
}

const REFERENCE_TOKENS = collectReferenceTokens();

function sentenceUsesKnownVocabulary(sentence: string, language: "nl" | "en") {
  const tokens = sentence
    .toLowerCase()
    .match(/[\p{L}]+(?:[-'][\p{L}]+)*/gu) ?? [];
  const vocab = language === "nl" ? REFERENCE_TOKENS.dutch : REFERENCE_TOKENS.english;
  const functionWords = language === "nl" ? DUTCH_FUNCTION_WORDS : ENGLISH_FUNCTION_WORDS;
  return tokens.every((token) => vocab.has(token) || functionWords.has(token));
}

function sentenceIsReferenceComplete(sentence: SentencePayload) {
...
function sentencePassesHardRules(sentence: SentencePayload, intent?: any): boolean {
  ...
  const verb = intent?.verb?.infinitive as string | undefined;
  ...
  if (verb) {
    ...
    if (collocation?.allowedSubjectTypes && intent?.subject?.semanticType && !collocation.allowedSubjectTypes.includes(intent.subject.semanticType)) {
      return false;
    }
    ...
    if (verb === "luisteren") {
      ...
      if (!/(muziek|radio|podcast|vogels)/.test(nl)) return false;
    }

    if (["nemen", "doen", "vinden"].includes(verb)) {
      const allowedObjectsByVerb: Record<string, string[]> = {
        nemen: ["trein", "bus", "beslissing", "pauze"],
        doen: ["huiswerk", "boodschappen", "werk"],
        vinden: ["boek", "sleutels", "cadeau", "restaurant"]
      };
      const allowed = allowedObjectsByVerb[verb] ?? [];
      if (!allowed.some((object) => nl.includes(object))) return false;
    }

    if (verb === "zeggen") {
      const hasClauseMarker = /\b(dat|of|"|')\b/.test(nl);
      const hasObject = /(iets|het|hem|haar|hun|mij|jou|ons|jullie)/.test(nl);
      if (!hasClauseMarker && !hasObject) return false;
    }

    if (collocation?.requiresObjectOrClause) {
      const hasClauseMarker = /\b(dat|of|omdat|terwijl|toen)\b/.test(nl);
      const hasObjectToken = intent?.object?.word ? nl.includes(String(intent.object.word).toLowerCase()) : false;
      if (!hasClauseMarker && !hasObjectToken) return false;
    }
  }

  if (intent?.location?.noun?.word) {
    const locationWord = intent.location.noun.word;
    if (locationWord === "school" && !nl.includes("op school")) return false;
    if (locationWord === "kantoor" && !nl.includes("op kantoor") && !nl.includes("op het werk")) return false;
    if (locationWord === "werk" && !nl.includes("op het werk")) return false;
    if (locationWord === "huis" && !nl.includes("thuis")) return false;
  }

  // Basic English/Dutch slot sync: if the sentence carries a concrete object or location in the intent,
  // both sides should still mention it after generation.
  if (intent?.object?.word) {
    const objectWord = String(intent.object.word).toLowerCase();
    const objectEnglish = String(intent.object.english ?? "").toLowerCase();
    if (!nl.includes(objectWord)) return false;
    if (objectEnglish && !en.includes(objectEnglish.replace(/^the\s+/, ""))) return false;
  }
...
function buildSentenceOnce(category: PracticeCategory): SentencePayload | null {
  ...
  switch (category) {
    ...
    case "perfect-tense": {
      ...
      const nl = useAdverb ? `${s.nl} ${hebben} ${adverb.nl} ${p.nl}.` : `${s.nl} ${hebben} ${p.nl}.`;
      const sentence: SentencePayload = {
        en: `${s.en} ${englishHave(s)} ${p.en}.`,
        nl,
        accepted: useAdverb ? uniqueSentences([nl, `${s.nl} ${hebben} ${p.nl} ${adverb.nl}.`]) : undefined,
        hint: getSubjectHint(s, hebben) || (useAdverb ? `Place "${adverb.nl}" before the past participle.` : "Perfect tense uses hebben/zijn + past participle."),
...
function buildSentence(category: PracticeCategory): SentencePayload {
  ...
  return buildSentenceOnce(category) ?? {
    en: "I am ready.",
    nl: "Ik ben klaar.",
...
export function createPracticeItem(direction: Direction, category: PracticeCategory): PracticeItem {
  ...
  const accepted = sentence.accepted?.length ? sentence.accepted : [
    expected,
    expected.replace("jij", "je"),
    expected.replace("you", "you all"),
